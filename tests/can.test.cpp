...
namespace hal::mock {
namespace {
auto error_callback(hal::mock::can& mock)
{
  return [&mock]() { throw hal::operation_not_supported(&mock); };
}
}  // namespace
void can_mock_test()
{
  ...
  "hal::can::configure tests"_test = []() {
    ...
    mock.spy_configure.trigger_error_on_call(3, error_callback(mock));
    ...
    [[maybe_unused]] auto f = throws<hal::operation_not_supported>(
      [&]() { mock.configure(expected2); });
  ...
  "hal::can::send tests"_test = []() {
    ...
    constexpr hal::can::message_t expected2{ .id = 2,
                                             .payload = { 'b' },
                                             .length = 1 };
    mock.spy_send.trigger_error_on_call(3, error_callback(mock));
